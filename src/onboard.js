...
console.log(`[auto-onboard] directory created`);
  ...
  try {
    ...
    // together-api-key is natively supported by CLI v2026.2.22+
    // No fallback needed.

    // Vertex AI uses OAuth (no API key). Onboard as openai with a dummy key
    // so OpenClaw creates its config. bootstrap.mjs then overrides the provider
    // with the real Vertex AI proxy config.
    if (PROVIDERS_WITHOUT_API_KEY.has(AI_PROVIDER) && !effectiveKey) {
    ...
    const payload = {
      flow: "quickstart",
      authChoice,
      authSecret: effectiveKey || "vertex-proxy-handles-auth",
    };
