...
export function createSetupRouter() {
  ...
  /**
   * POST /setup/api/patch-model — Patch model config directly in openclaw.json.
   * Body: { model?: string, baseUrl?: string, provider?: string, removeProviders?: string[] }
   */
  router.post("/api/patch-model", requireSetupAuth, async (req, res) => {
    try {
      ...
      // Remove provider entries without "models" array (validator requires it)
